...
headers = {
    "X-RapidAPI-Key": "a134cbb223msh6c78a05b63fa742p1a1f2ejsn7a88f409fec4",
...
# ||__||__||__||__||__||here start all the function of mode||__||__||__||__||__||__||__||
...
# ||__||__||__||__||__||__||here close the function of mode||__||__||__||__||__||__||__||
...
def query(update: Update, contex):
    try:
        ...
        if "d" in data:
            ...
            if movies:
                ...
                title = first_movie.get("l")
                year = first_movie.get("y")
                rating = first_movie.get("q")
                genres = first_movie.get("s", [])
                rank = first_movie.get("rank")
                id = first_movie.get("id")
                print("Title:", title)
                print("Year:", year)
                print("Rating:", rating)
                print("Genres:", genres)
                ...
                image_url = first_movie.get("i", {}).get("imageUrl")
                if image_url:
                    try:
                        print("Image URL:", image_url)
                        contex.bot.send_photo(
                            chat_id=update.effective_chat.id, photo=image_url, caption=caption)
                    except Exception as e:
                        print(e)
                else:
